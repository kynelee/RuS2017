...
encoded_public_key = skbob.public_key.encode(encoder =
        nacl.encoding.HexEncoder)

try: 
        decoded_public_key = nacl.public.PublicKey(encoded_public_key,
...
# Bob wishes to send Alice an encrypted message so Bob must make a
# Box with
...
# Note the "b" in the front of the string means to keep the string
# in
...
# This is a nonce, it *MUST* only be used once, but it is not
# considered
#   secret and can be transmitted or stored alongside the
#   ciphertext. A
...
# Alice creates a second box with her private key to decrypt the
# message
alice_box = Box(skalice, pkbob)

# Decrypt our message, an exception will be raised if the encryption
# was
